...
Complemento de `01-DOCUMENTACION.md`. Aquí va: (1) qué tan bien está la estructura frente a las convenciones de **Django + DRF**, (2) **bugs** detectados, (3) mejoras de **lógica del admin** para ayudar al cliente, (4) foco en **inventario/máquinas**, y (5) el **apartado de refacciones a futuro**.
...
### Lo que está bien 🟢
- **Apps por dominio** (`inventario`, `renta`, `ventas`, `empresas`, `refacciones`) — buena separación por responsabilidad.
- **`Equipo` (catálogo) ≠ `Inventario` (unidad física)** — modelado correcto para rentar/vender la *misma* máquina varias veces y trazar cada unidad.
- **Dinero en `DecimalField`** (no floats) — correcto.
- **Transacciones atómicas** y `select_for_update()` en las operaciones críticas (crear renta, vender, mantenimiento) — muy bien.
- **`db_table`, `verbose_name`, `ordering`** definidos — admin en español y prolijo.
- **JWT con vida larga configurable**, cache preparada para Redis, storages condicionales (Cloudinary/WhiteNoise) — pensado para producción.

### Desviaciones respecto a la convención 🟡

1. **App "grande" `maquinaria`.** Hoy concentra catálogo **+** autenticación/perfil **+** notificaciones **+** soporte **+** cupones **+** dashboard. La convención Django es *una app = una responsabilidad*. Sugerido dividir a futuro:
...
4. **Todo son function-based views + `@api_view` sueltos.** Es válido, pero para CRUD con filtros lo idiomático en DRF es **`ViewSet` + `Router`** (menos boilerplate, rutas consistentes, paginación gratis). Ver §4.
...
7. **Tests vacíos.** `tests.py` existe en cada app pero sin pruebas. Para la lógica de estados (que es el corazón del negocio) faltan tests — es justo donde más ayudan.
...
- `SECRET_KEY` tiene fallback embebido → en prod debe venir **solo** de entorno (fallar si no está).
...
El **Django admin** ya es la herramienta operativa; con pocos cambios rinde mucho más para mostrador/almacén:

1. **Acciones masivas** (`admin actions`) en `Inventario`: "Enviar a mantenimiento", "Sacar de mantenimiento", "Marcar disponible" para varias unidades a la vez.
2. **Filtro y color para `mantenimiento`** (hoy falta) y filtro por `condicion` y por `ubicacion_actual`.
3. **`list_select_related`** en los admins (`Inventario`, `Renta`, `Venta`) para evitar N+1 al pintar la lista.
4. **Inline de `Inventario` dentro de `Equipo`**: ver/crear unidades desde la ficha del modelo.
5. **Dashboard real** (`/api/dashboard/metricas/` hoy devuelve `orders: 0, revenue: 0`). Cablearlo a datos reales:
   - Ingresos por rentas activas y ventas del mes.
   - Unidades por estado (disponible/rentado/mantenimiento).
   - Rentas por vencer / vencidas.
   - Top equipos más rentados. Estas KPIs son justo lo que el dueño quiere ver.
6. **Semáforo de rentas vencidas** en el `list_display` de `Renta` (ya tienes `vencida`; agrégalo como badge de color).
7. **Validación de precios** ≥ 0 (hoy se pueden capturar negativos): `MinValueValidator(0)` en los `DecimalField` de precio.
8. **Historial/auditoría**: registrar quién cambió el estado de una unidad y cuándo (con `django-simple-history` o un modelo `MovimientoInventario`). Da trazabilidad, que es un principio del propio `PRODUCT.md`.
...
1. **Historial de mantenimiento como modelo, no solo un estado.** Hoy `mantenimiento` es un flag sin registro. Crear:
   ...
   ```
   Permite: costo acumulado por máquina, tiempo fuera de servicio, y **mantenimiento preventivo por horas/uso**.
...
3. **Costo y rentabilidad por unidad**: agregar `precio_compra` y `fecha_adquisicion`. Con eso el dashboard calcula **ROI por máquina** (cuánto ha generado en rentas vs. lo que costó) — oro puro para decidir qué reponer o dar de baja.
...
- **Ligar refacciones a máquinas**: tabla de compatibilidad `RefaccionEquipo` (o M2M `Refaccion.equipos_compatibles = M2M(Equipo)` / por `Marca`). Así, desde una máquina se ven "sus" refacciones.
...
- **Proveedor** (`Proveedor` + `costo_compra`) para calcular margen y saber a quién comprar.
- **Ubicación en almacén** (`ubicacion`, pasillo/estante) para el mostrador.

### Fase B — API y flujo
- **Exponer API** (`refacciones/urls.py` hoy está deshabilitado a propósito): CRUD + `search` por `codigo_barras` (para el lector) + endpoint de "bajo stock".
...
- **Reponer stock al cancelar/devolver** una venta (hoy no ocurre).
...
- Vincular una refacción a un **`Mantenimiento`** (qué piezas se usaron en cada servicio) — cierra el ciclo con §5.
...
- **Frontend:** decidir sobre rutas no enganchadas (`Cart`, `Checkout`, `Cotizacion`, `EquipoDetail`, `Profile`, `AdminDashboard`) y sobre la plantilla `scenes/*` + `mockData.js` — o se cablean o se borran.
...
> Regla práctica: **Fase 0 antes de cualquier deploy nuevo.** Lo demás se puede priorizar según lo que más duela en la operación diaria.
