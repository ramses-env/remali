...
Al crear la renta se toma un snapshot del precio del catálogo, que no se recalcula si el
...
- `recargo` se calcula solo en la devolución si `fecha_devolucion_real > fecha_fin`:
  `recargo = tarifa_diaria × días_de_retraso`, y la tarifa diaria se deriva de la modalidad.
- `deposito` (la garantía) se guarda como referencia.
...
Los precios se capturan con IVA incluido y el sistema los desglosa:
...
- El ticket ahora incluye la línea de la máquina y el desglose de IVA.
...
| GET | `rentas/<id>/ticket/` | Comprobante de renta en PDF (58 mm). |
...
| GET | `ventas/<id>/ticket/` | Ticket de venta en PDF (58 mm). |
...
## 9. Tickets y comprobantes (impresión)

Al registrar una renta o venta, el comprobante se muestra dentro del sistema, en un modal, sin abrir
otra pestaña. También se reimprime después con el botón "Ticket" en las listas de rentas y ventas.

El contenido sale de un solo lugar: `apps/renta/comprobante.py` y `apps/ventas/comprobante.py`
(`datos_comprobante_*`) producen un JSON que alimenta tanto el modal como el PDF.

- En renta: equipo y unidad, cliente/empresa/obra, periodo, y el desglose de dinero
  (precio, subtotal, descuento, recargo, total, depósito).
- En venta: máquina o refacciones, subtotal, IVA (16 %) y total, y método de pago.
...
Lo clave de la impresión térmica de 58 mm: el modal (`src/components/TicketModal.tsx`) se monta con un
...
Así el navegador usa el ancho de rollo de 58 mm en vez de Letter, y la térmica imprime correcto.
...
| El estado se escribía en 3 lugares | Fuente única en `Inventario`; señales vaciadas; las vistas no tocan `estado`. |
| No había reservas ni validación de fechas | Estado `reservada` y anti-traslape por rango de fechas. |
...
| Cliente solo como texto libre | Ligado opcional a `Empresa` y `Obra`, con fallback a texto. |
...
- RentModal: empresa y obra (selects que cargan de `/empresas/`), fecha de inicio (si es futura se
  guarda como reserva), descuento y depósito. El título y el botón cambian a "Reservar".
- SellModal: empresa, teléfono y vista previa de subtotal, IVA y total.
- RentasAdmin: filtro de 4 estados (activas, reservas, finalizadas, canceladas), muestra
  empresa y total, y saca los botones de devolver y cancelar según el estado.
- VentasAdmin: muestra IVA y estado, con botón para cancelar, que revierte stock y máquina.
- MaintModal: envía `forzar` al mandar a taller una unidad rentada, y así evita el 409.
