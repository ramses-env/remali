...
export default function ProductList() {
  ...
  useEffect(() => {
    ...
    const conditions = filters['condition']?.filter(Boolean) || []
    ...
    api.get<Product[]>(`/products/?${qs}${search}`)
      .then(r => setItems(r.data))
      .catch(() => setItems([
        { id: 1, title: 'Producto demo', price: 49.9, image: '/vite.svg' },
        { id: 2, title: 'Producto premium', price: 99.0, image: '/vite.svg' },
      ]))
  }, [filters, query])
  ...
  const shown = useMemo(() => {
    ...
    function score(p: Product) {
      ...
      const d = norm((p as any).description || '')
      const b = norm((p as any).brand || '')
      const c = norm((p as any).category || '')
      const full = `${t} ${d} ${b} ${c}`.trim()
      if (!q) return 0
      let s = 0
      if (full === q) s += 30
      if (t.startsWith(q)) s += 16
      if (full.includes(q)) s += 10
      if (d.includes(q)) s += 6
      if (b.includes(q) || c.includes(q)) s += 8
      const tokens = q.split(/\s+/).filter(Boolean)
      function ed(a: string, b: string) {
      ...
      const words = full.split(/\W+/).filter(Boolean)
      tokens.forEach(tok => {
        if (full.includes(tok)) s += 6
        else {
          if (tok.length <= 6) {
            let best = Infinity
    ...
    const queryConds: string[] = []
    q.split(/\s+/).forEach(tok => {
      if (tok === 'nuevo') queryConds.push('new')
      if (tok === 'usado' || tok === 'segunda' || tok === 'segunda-mano') queryConds.push('used')
    })
    if (queryConds.length) {
      const set = new Set(queryConds)
      arr = arr.filter(p => {
        const c = (String((p as any).condition || '')).toLowerCase()
        if (!c) return true
        return set.has(c)
      })
    }
  ...
  }, [items, sortBy, order, pageSize, page, query])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center space-y-2">
        <p className="uppercase tracking-widest text-sm text-gray-500">Bienvenido a</p>
                    ...
                    onKeyDown={e => {
                      if (e.key === 'ArrowDown') { e.preventDefault(); setActiveSuggestion(i => Math.min(i + 1, suggestions.length - 1)) }
                        ...
                        if (activeSuggestion >= 0 && suggestions[activeSuggestion]) {
                          setInputValue(suggestions[activeSuggestion].title)
                          setQuery(suggestions[activeSuggestion].title)
                          setShowSuggestions(false)
                          navigate(`/producto/${suggestions[activeSuggestion].id}`)
                        } else {
                          setQuery(inputValue)
                          setShowSuggestions(false)
                        }
                    ...
                    <div className="absolute left-10 right-0 top-11 z-20 rounded-b-lg bg-white border border-neutral-200 shadow-lg max-h-64 overflow-auto">
                      {suggestions.map((s, i) => (
                        <button
                          ...
                          onClick={() => { setInputValue(s.title); setQuery(s.title); setShowSuggestions(false); navigate(`/producto/${s.id}`) }}
                        >
                          {s.image && (
                            <img
                              src={s.image}
            ...
            <div ref={gridRef} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {shown.map((p, i) => (
                <div key={p.id} ref={i === 0 ? firstCardRef : undefined}>
                  <ProductCard id={p.id} title={p.title} price={p.price} image={p.image || ''} subtitle={p.description?.slice(0, 40)} rating={(p.id % 5) + 1} />
                </div>
              ))}
            </div>
