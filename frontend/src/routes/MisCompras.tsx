  ...
  return (
    <div style={{ animationDelay: `${i * 40}ms` }} className="stagger-item rounded-2xl border border-edge bg-surface p-5">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <span className={`text-[11px] font-semibold uppercase tracking-wide px-2.5 py-1 rounded-full ${cancelada ? 'bg-red-500/10 text-red-500' : apartada ? 'bg-gold/15 text-gold' : 'bg-libre/10 text-libre'}`}>
          ...
          {abonos > 0 && (
            <>
              <button onClick={() => setAbierto(v => !v)}
                className="mt-1.5 inline-flex items-center gap-1 text-[12px] font-bold text-gold hover:opacity-80 transition-opacity">
          ...
          <div className="flex items-center gap-1 text-[10.5px] font-bold">
            {([['confirmado', 'Confirmado'], ['en_camino', 'En camino'], ['en_sucursal', 'En sucursal'], ['entregado', 'Entregado']] as [string, string][]).map(([f, lbl], idx) => (
              <span key={f} className="flex items-center gap-1 min-w-0">
                ...
                <span className={`px-2 py-0.5 rounded-full whitespace-nowrap ${idx <= faseIdx ? 'bg-gold/20 text-gold' : 'bg-surface-2 text-mute'}`}>{lbl}</span>
              </span>
            ))}
          </div>
